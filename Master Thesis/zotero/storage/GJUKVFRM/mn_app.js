...
var OmniSearch = {
  ...
  loadSearchData: function() {  
    ...
    request.open('GET', '/js/search_data.json', true);

    request.onload = function() {
      if (request.status >= 200 && request.status < 400) {
        OmniSearch.searchData = JSON.parse(request.responseText);
      } else {
  ...
  search: function(query) {
    ...
    var results = this.searchData.filter(function(entry) {
      return entry.text.match(re);
    });

    return this.formattedResults(results);
  },
  formattedResults: function(results) {
    var people = results.filter(function(entry) { return entry["type"] == "partner"; });
    var perspectives = results.filter(function(entry) { return entry["type"] == "perspective"; });
    var companies = results.filter(function(entry) { return entry["type"] == "company"; });
    var text = "";

    if(people.length == 0 && perspectives.length == 0 && companies.length == 0) {
      text = "<strong>No Matching Results</strong>"
    }

    if(people.length > 0) {
      text += "  <strong>People</strong><ul>"
      for(var i =0; i<people.length; i++) {
        var p = people[i];
        text += "<li><a href='" + p.url + "'>" + p.text + "</a></li>"
      }
      text += "</ul>"
    }

    if(perspectives.length > 0) {
      text += "  <strong>Perspectives</strong><ul>"
      for(var i =0; i<perspectives.length && i < 7; i++) {
    ...
    if(companies.length > 0) {
      text += "  <strong>Partnerships</strong><ul>"
      for(var i =0; i<companies.length && i<10; i++) {
        var p = companies[i];
        text += "<li><a href='" + p.url + "' target='_blank' rel='noopener'>" + p.text + "</a></li>"
      }
      text += "</ul>"
    }
...
function submitOmniSearch(text) {
  selectedIndex = -1;
  var formattedResults = OmniSearch.search(text);
  $('#omni-search-results').html(formattedResults);
}

function selectItem(index) {
  selectedIndex = index;
  var lis = $("#omni-search li");
  ...
  $(lis[selectedIndex]).addClass("active");
};


$(document).ready(function() {
  OmniSearch.loadSearchData();

  $("#fake-omni-search").on("focus", function() {
    $("body").addClass("omni-search-window--active");
    // $("html").addClass("noscroll");
    $("#omni-search-input").focus();
    // window.scrollTo(0, 0);
  })

  $(".close-omni").on("click", function() {
    $("body").removeClass("omni-search-window--active");
    $("html").removeClass("noscroll");
  })
  //
  $("#omni-search-input").on("focus", function() {
    setTimeout(function() {
      $(window).scrollTop(0);
      $("html").addClass("noscroll");
    }, 300);
  ...
  $("#omni-search-input").on("keydown", function(ev) {
    var keyCode = ev.keyCode || ev.which;
    var arrow = {left: 37, up: 38, right: 39, down: 40, enter: 13};
    if(keyCode == arrow.up || keyCode == arrow.down) { return; }
    if(keyCode == arrow.enter) {
      if($("#omni-search li a").length == 1) {
        document.location = $("#omni-search li a").attr("href");
      }
    }
 // s   ev.preventDefault();
    ...
    clearTimeout(omni.data('timeout'));

    omni.data('timeout', setTimeout(function() {
      var text = omni.val();
      submitOmniSearch(text);
    }, 300));
  });


  $("#omni-search").on("keydown", function(ev) {
    var keyCode = ev.keyCode || ev.which;
    var arrow = {left: 37, up: 38, right: 39, down: 40, enter: 13 };
    switch (keyCode) {
      case arrow.up:
      ...
      case arrow.enter:
        if(selectedIndex == 0 || selectedIndex > 0) {
          document.location = $("#omni-search li.active a").attr("href");
        }
        ev.preventDefault();
        break;
    }
...
function closeCaseStudyModal() {
  $modalContent = $("#case-study-modal-container #case-study-article");
  $("#case-study-modal-container").removeClass("opened");
  setTimeout(function() {
   $modalContent.remove()
 }, 500);

 // if fullpage plugin is enabled
...
function openCaseStudyModal(modalId) {
  setTimeout(function() {
...
$(document).ready(function() {
  $("#home .open-case-study, #partnerships .open-case-study").on('click', function(e) {
    var caseStudyURL = $(this).attr('href');
    var modal = "#case-study-modal-container";
    $(modal + " #case-study-content").load(caseStudyURL + " #case-study-article");
    openCaseStudyModal(modal);
    e.preventDefault();
  })

  $("#case-study-modal-container").on('click', '.close-modal', function() {
    closeCaseStudyModal();
  })
});


// Actions that should be bound on each page load or on each page partial refresh
function pageActionBindings() {
  ...
  // don't display tooltips on iOS
  var userAgent = window.navigator.userAgent;
  if ( userAgent.match(/iPad/i) || userAgent.match(/iPhone/i) ) {
  } else {
    $('#partnerships .tooltip-trigger').tooltip({
      trigger: 'hover',
      placement: 'top'
    })
  }
  ...
  $('#article .article__body iframe').each(function() {
    $(this).wrap('<div class="blog-body-embed"></div>');
  });

  // close omni-search inside navbar when navbar is collapsed
  $('#main-nav').on('hidden.bs.collapse', function () {
    var activeOmniSearch = "omni-search-window--active";

    if ( $("." + activeOmniSearch).length ) {
      console.log("collapsed omnisearch");
      $("." + activeOmniSearch).removeClass(activeOmniSearch)
    }
  })

  // START: expand "more" in header
  $(".js--more-expand.inactive a").on('click', function(e) {
    e.preventDefault()
  })

  $(".js--more-expand").on('click', function(e) {
    e.preventDefault()
    $(this).toggleClass('active');
    $("#js--more-expand").toggleClass('inactive');
  })
  // END: expand "more" in header
}
